...
public class FileDataHandler
{
  ...
  public GameData Load(string profileId, bool allowRestoreFromBackup = true)
  {
    ...
    if (File.Exists(fullPath))
    {
      ...
      catch (Exception e)
      {
        ...
        {
          Debug.LogWarning(
            "Failed to load data file. Attempting to roll back.\n" + e
          );
        ...
        {
          Debug.LogError(
            "Error occured when trying to load file at path: "
              + fullPath
  ...
  public void Save(GameData data, string profileId)
  {
    ...
    try
    {
      ...
      {
        throw new Exception(
          "Save file could not be verified and backup could not be created."
        );
      }
    }
    catch (Exception e)
  ...
  public void Delete(string profileId)
  {
    ...
    try
    {
      ...
      {
        Debug.LogWarning(
          "Tried to delete profile data, but data was not found at path: "
            + fullPath
        );
      }
    }
  ...
  {
    Dictionary<string, GameData> profileDictionary =
    ...
    foreach (DirectoryInfo dirInfo in dirInfos)
    {
      ...
      {
        Debug.LogWarning(
          "Skipping directory when loading all profiles because it does not contain data: "
            + profileId
        );
        continue;
      }
      ...
      {
        Debug.LogError(
          "Tried to load profile but something went wrong. ProfileId: "
            + profileId
        );
      }
    }
    ...
    foreach (KeyValuePair<string, GameData> pair in profilesGameData)
    {
      ...
      else
      {
        DateTime mostRecentDateTime = DateTime.FromBinary(
          profilesGameData[mostRecentProfileId].lastUpdated
        );
  ...
  private string EncryptDecrypt(string data)
  {
    ...
    {
      modifiedData += (char)(
        data[i] ^ encryptionCodeWord[i % encryptionCodeWord.Length]
      );
    }
    return modifiedData;
  }
  ...
  private bool AttemptRollback(string fullPath)
  {
    ...
    try
    {
      ...
      if (File.Exists(backupFilePath))
      {
        ...
        success = true;
        Debug.LogWarning(
          "Had to roll back to backup file at: " + backupFilePath
        );
      }
      ...
      {
        throw new Exception(
          "Tried to roll back, but no backup file exists to roll back to."
        );
      }
    }
    catch (Exception e)
    {
      Debug.LogError(
        "Error occured when trying to roll back to backup file at: "
          + backupFilePath
          + "\n"
          + e
      );
